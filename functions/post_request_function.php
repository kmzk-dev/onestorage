...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($target_dir_path === false || strpos($target_dir_path, DATA_ROOT) !== 0) {
        ...
        switch ($action) {
            ...
            case 'rename_item':
                ...
                if (!$old_path || strpos($old_path, $target_dir_path) !== 0) { $_SESSION['message'] = ['type' => 'danger', 'text' => '元のアイテムが見つかりません。']; break; }
                if (empty($new_name_input) || strpbrk($new_name_input, "\\/?%*:|\"<>") !== false || str_starts_with($new_name_input, '.')) { $_SESSION['message'] = ['type' => 'danger', 'text' => '無効な名前です。記号や.で始まる名前は使えません。']; break; }
                $final_new_name = '';
                if (is_dir($old_path)) { $final_new_name = $new_name_input; } 
                else { $extension = pathinfo($old_name, PATHINFO_EXTENSION); $final_new_name = empty($extension) ? $new_name_input : $new_name_input . '.' . $extension; }
                ...
                break;
            case 'move_items':
                $items_to_move = json_decode($_POST['items_json'] ?? '[]'); $destination_rel_path = $_POST['destination'] ?? ''; $destination_abs_path = realpath(DATA_ROOT . '/' . $destination_rel_path);
                if ($destination_abs_path === false || strpos($destination_abs_path, DATA_ROOT) !== 0) { $_SESSION['message'] = ['type' => 'danger', 'text' => '不正な移動先です。']; break; }
                $success_count = 0; $error_count = 0;
                foreach ($items_to_move as $item_name) {
                    $source_path = $target_dir_path . '/' . $item_name; $dest_path = $destination_abs_path . '/' . $item_name;
    ...
    header('Location: ?path=' . urlencode($path_from_form)); exit;
}