...
/**
 * Base32デコード
 * Google Authenticatorの秘密鍵はBase32でエンコードされている
 */
function base32_decode($base32) {
    ...
    for ($i = 0; $i < $length; $i++) {
        $char = $base32[$i];
        if ($char == '=') break; // パディング文字
        $char_value = strpos(BASE32_CHARS, $char);
        if ($char_value === false) continue; // 無効な文字はスキップ
...
/**
 * 新しいMFAシークレットキーを生成する
 * @param int $length 秘密鍵の長さ（Base32文字数、通常16文字）
...
/**
 * TOTPコードを検証する
 * @param string $secret Base32エンコードされた秘密鍵
 * @param string $code ユーザーが入力した6桁のコード
 * @param int $time_drift 許容する時間ウィンドウのズレ（デフォルト: 1, 30秒ステップなら前後30秒）
